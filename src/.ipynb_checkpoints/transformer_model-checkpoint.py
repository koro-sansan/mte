...
class Embedding_MorphTE(nn.Cell):
    """
    A embeddings lookup table with a fixed dictionary and size.
    ...
    def __init__(self,
        ...
        super(Embedding_MorphTE, self).__init__()

        self.vocab_size = vocab_size
        self.embedding_size = embedding_size

        assert morInfoPath is not None
        self.co_matrix, self.num_mor = self.get_morphInfo(morInfoPath)

        self.rank = rank
        self.order = order
        self.core_dim = math.ceil((embedding_size) ** (1 / order))
        assert order == 3

        self.weight1 = Parameter(normal_weight([self.rank, self.num_mor, self.core_dim], self.core_dim))
        self.weight2 = Parameter(normal_weight([self.rank, self.num_mor, self.core_dim], self.core_dim))
        self.weight3 = Parameter(normal_weight([self.rank, self.num_mor, self.core_dim], self.core_dim))
    ...
    def get_morphInfo(self, mor_path):

        mor_res = json.load(open(mor_path))
        wordId2mor = mor_res['wordId2mor']
        mor_set = ['[mor-PAD-1]', '[mor-PAD-2]'] + mor_res['mor_set']
        ...
        for k, v in wordId2mor.items():
            ...
            assert len(mor_li) == 3
            co_matrix.append([mor2id[mor] for mor in mor_li])

        return Tensor(np.array(co_matrix), dtype=ms.int32), len(mor2id)

    def get_ith_core(self, ith, weight):
        # co_inx = self.co_matrix[:, ith].unsqueeze(0).unsqueeze(-1).expand(self.rank, -1, self.core_dim)

        ith = ith - 1
        co_inx = self.expand(self.expand(self.co_matrix[:, ith], 0), -1)
        # print(co_inx.shape) (    1, 10149,     1)
        co_inx = ops.BroadcastTo(shape=(self.rank, self.vocab_size, self.core_dim))(co_inx)
        # print(co_inx.shape) (    4, 10149,     8)

        w_slice = self.gatherD(weight, 1, co_inx)
        # print(w_slice.shape) # (    4, 10149,     8)
        # print()

        return w_slice

    def get_emb_matrix(self):
        w1 = self.get_ith_core(1, self.weight1)
        w2 = self.get_ith_core(2, self.weight2)
        w3 = self.get_ith_core(3, self.weight3)

        w = w1[:, :, :, None] * w2[:, :, None, :]
        # w = self.array_mul(self.expand(w1, ), self.expand)
        w = w.view(self.rank, self.vocab_size, -1)
        ...
        w = w[:, :self.embedding_size]

        w = self.ln(w)

        return w #self.embedding_table
    ...
    def construct(self, input_ids):
        ...
        embedding_table = self.get_emb_matrix()
...
class TransformerModel(nn.Cell):
    ...
    def __init__(self,
        ...
        if config.morphte == "true":
            self.tfm_embedding_lookup = Embedding_MorphTE(
                vocab_size=config.vocab_size,
                embedding_size=self.embedding_size,
                rank=config.rank,
                morInfoPath=config.morInfoPath
            )
        else:
