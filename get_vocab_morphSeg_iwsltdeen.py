...
vocab_dict = load_vocab_file('./data/iwslt14.de-en.bpe10000/vocab_deen.txt')
...
for id, w in tqdm(inv_vocab_dict.items()):
    ...
    if w.endswith('@@'):
        lang = 'en' if word_check_ait(w[:-2]) else 'de'
        mors = mor_func(w[:-2], lang=lang) + ['@@']
    else:
        lang = 'en' if word_check(w) else 'de'
...
json.dump(bpe_mor_res, open('./data/iwslt14.de-en.bpe10000/' + 'deen-jointed_morSeg_results.json', 'w', encoding='utf-8'), ensure_ascii=False, indent=2)
